...
const useSocket = (userId: string): UseSocket => {
  const [messages, setMessages] = useState<Message[]>([]);
  const [isConnected, setIsConnected] = useState(false);
  ...
  useEffect(() => {
    ...
    if (!socketRef.current) {
      ...
      socketRef.current.on("matchFound", ({ matchUser }) => {
  ...
  const sendMessage = useCallback((receiverId: string, message: string) => {
    if (!message.trim() || !receiverId || !isConnected) return;
  ...
  return { messages, sendMessage, isConnected, matchUser };
};
