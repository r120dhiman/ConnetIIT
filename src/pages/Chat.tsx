    ...
    <div className="max-w-6xl mx-auto h-[calc(100vh-4rem)] flex flex-col">
      ...
      {selectedUser ? (
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="p-4 bg-[#1B1730] border-t border-[#262438]">
            <div className="flex items-center">
              <input
                type="text"
                value={messageText}
                onChange={(e) => setMessageText(e.target.value)}
                placeholder="Type a message..."
                className="flex-grow p-2 rounded-l-md bg-[#262438] text-white"
                onKeyPress={(e) => e.key === 'Enter' && handleSendMessage()}
              />
              <button 
                onClick={handleSendMessage}
                className="p-2 bg-[#E76F51] text-white rounded-r-md h-full"
              >
